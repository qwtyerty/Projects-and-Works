...
/**
	Processes .job files from a fifo pipeline which is managed by a separate bash script, 
		job files contains lines of intsructions to call files via exec to run prewritten 
		excutible files which enact the specified instruction onto a .tga image and uses 
		fork for splitting processes 
**/
...
int main(int argc, char* argv[])
{
	...
	**/
	...
	//Make an endless loop that only child processes can break out of once they have a job
	while(true)
	{
		if(fifo.eof())
			fifo.clear();
		if(getline(fifo, job))
		{
			//File found from fifo read then have child break off
			p = fork();
			if(p == 0)
				runChild(const_cast<char*>(job.c_str()), inFolder, argv[3]);
		}
	}
